...
export const PackageUploadButton: React.FC<Prop> = (prop: Prop) => {
  ...
  const handleFileUpload = async () => {
    try {
      ...
      console.log(`done1 >> response`, response);
    } catch (error: any) {
      const errMsg = error?.name === 'AxiosError' 
        ? error?.response?.data?.message 
        : error?.message;
      const errErrors = error?.name === 'AxiosError' 
        ? error?.response?.data?.errors 
        : error?.errors;
      console.log(`error2 >> response`, error?.response);
      console.log(`error2 >> error`, error);
      console.log(`error2 >> cname`, error?.constructor.name);
